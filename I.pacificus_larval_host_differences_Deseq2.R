...
# input: path to directory which contains (at any level) counts files from the same experiment (and only the same experiment)
...
mappingDir <- '/path/to/working/directory'
# point to directory with raw read counts, in this case called `counts_for_deseq`
quantDir <- file.path(mappingDir, 'counts_for_deseq')
